<script>
    ...
    onMount(async () => {
            ...
            colorScaleMariage = scaleSequential(interpolate("white", "#802780")).domain([0, maxMariages])
            colorScaleConversions = scaleSequential(interpolate("white", "#802780")).domain([0, maxConversions])
...
<main>
    ...
    <div id="map">
        ...
        {#each geodata as kanton, i}
            <div id={kanton.properties.canton}
                 style="position: absolute; display: none; color: #802780; background-color: whitesmoke; opacity: 0.6; padding: 20px; border-radius: 10%;">
                <h5 style="color:{color}">{kanton.properties.canton}</h5>
                Femme-Femme: <b>{kanton.properties.f_f}, conversion {kanton.properties.c_f_f} </b><br>
                Homme-Homme: <b>{kanton.properties.h_h}, conversion {kanton.properties.c_h_h} </b><br>
            </div>
        ...
        {/each}
            ...
            <Column>
                <div id="title">
                <h2>Oui, je le veux 2022</h2>

                </div>
                <div id="labels">
    <label>
        <input checked={selected==='total'} on:change={onChange} type="radio" name="mariage" value="total" /> Mariages
    </label>

    <label>
        <input checked={selected==='total_c'} on:change={onChange} type="radio" name="mariage" value="total_c" /> Conversion partenariat enregistré
    </label>
                </div>
            </Column>
        </Row>
    ...
    <footer><small><code>Données: <a href="https://www.bfs.admin.ch/bfs/fr/home/statistiques/population/mariages-partenaires-divorces/mariages.gnpdetail.2023-0216.html">Office fédéral de la statistique (OFS), Statistique du mouvement naturel de la
